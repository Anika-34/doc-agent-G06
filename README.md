...
- **Submit via GitHub:** this folder *is* your repo. Create a **public** GitHub repo, `git push`, and each milestone `git tag aN-submit && git push --tags`. Full steps: **`SUBMISSION.md`**. A private repo = no submission.
- **Read in order:** `handbook/01-START-HERE` → `02-How-To-Submit` → `03-Project-Specification` → `04-Project-Walkthrough` → `05-Codebase-Guide` → `06`-Group-Assignment-Workbook (domains, specialities, NFRs, sources, build buckets).
